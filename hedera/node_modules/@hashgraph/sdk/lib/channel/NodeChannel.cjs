      ...
      this._initializeClient().then(() => {
        ...
        this._client?.waitForReady(deadline, err => {
          if (err) {
            ...
            metadata.set("x-user-agent", _version.SDK_VERSION);
            this._client?.makeUnaryRequest(`/proto.${serviceName}/${method.name}`, value => value, value => value, Buffer.from(requestData), metadata, (e, r) => {
              callback(e, r);
            });
          }
        });
